...
class Penghasilan extends Admin_Controller {
	...
	{
		$this->title 	= lang("label_penghasilan");
		$this->assets 	= array('assets_index');
        $this->content  = "penghasilan/index";
        
        $params = [
            'action_home' => 'dashboard',
            'breadcrumb_active' => lang('label_penghasilan')
        ];
    ...
    public function ajax_list(){
        ...
        $list = $this->Api_affiliate->get_datatables_penghasilan($param);
        $data = array();
        $no = $_POST['start'];
        $dataTables = array();
        foreach ($list->result->data as $data) {
            ...
            $row[] = $data->Name;
            $row[] = $data->Email;
            $row[] = 'Rp'.number_format($data->Penghasilan);
            $row[] = 'Rp'.number_format($data->Dicairkan);
            $row[] = 'Rp'.number_format($data->BelumDicairkan);
            $dataTables[] = $row;
        }
