<!-- Content Header (Page header) -->
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1><?= title() ?></h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><?= anchor($action_home,lang('label_home')) ?></li>
                    <li class="breadcrumb-item active"><?= $breadcrumb_active ?></li>
                </ol>
            </div>
        </div>
    </div><!-- /.container-fluid -->
</section>

<!-- Main content -->
<section class="content">
    <div class="row">
        <div class="col-12">
            <?= show_alert() ?>
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"><?= lang('label_customer') ?></h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    <div class="form-group row">
                        <label class="col-sm-2 col-form-label"><?= lang('label_city') ?></label>
                        <div class="col-sm-10">
                            <?php echo form_dropdown('CityId',@$data['City'],'',array('id'=>'CityId', 'class'=>'form-control select2bs4','width'=>'100%'));?>
                        </div>
                    </div>
                    <table id="datatables" class="table table-bordered table-hover">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th><?= lang('label_nama') ?></th>
                                <th><?= lang('label_gender') ?></th>
                                <th><?= lang('label_email') ?></th>
                                <th><?= lang('label_whatsapp') ?></th>
                                <th><?= lang('label_status') ?></th>
                                <th><?= lang('label_complete_profile') ?></th>
                            </tr>
                        </thead>
                    </table>
                </div>
                <!-- <div class="card-footer">
                    <?= anchor($action_add,lang('label_add'),array('class'=>'btn btn-primary')) ?>
                </div> -->
            </div>
        </div>
    </div>
</section>
