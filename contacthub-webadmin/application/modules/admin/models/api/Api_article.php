...
class Api_article
{
    ...
    public function getAllNotPaginate($UserId,$filter=[]){
        try {
            $response   = $this->client->request('GET', '/article/all',[
                'headers'=> [
                    'Content-Type' => 'application/json',
    ...
    public function insert($UserId,$data=[]){
        try {
            $response   = $this->client->request('POST', '/article',[
                'headers'=> [
                    'X-Auth-UserId' => $UserId
                ],
                'multipart' => $data 
    ...
    public function update($UserId,$data=[]){
        try {
            $response   = $this->client->request('PUT', '/article',[
                'headers'=> [
                    'X-Auth-UserId' => $UserId
                ],
                'multipart' => $data 
    ...
    public function delete($data){
        try {
            $response   = $this->client->request('DELETE', '/article',[
                'headers'=> [
                    'Content-Type' => 'application/json'
                ],
                'json' => $data
