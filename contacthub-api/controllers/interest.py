...
class CreateInterestForm(Form):
    InterestId = StringField('InterestId', [validators.required()])
    
class DeleteInterestForm(Form):
    InterestId = StringField('InterestId', [validators.required()])
...
class InterestApi(Resource):
    def post(self) -> Response:
        try:
            ...
            form = CreateInterestForm().from_json(request.json)
            ...
            if not type(body) is dict:
                body = {}
            body['_id'] = body['InterestId']
            del body['InterestId']
            result = Interest.create(self,**body)
    ...
    def delete(self) -> Response:
        try:
            ...
            form = DeleteInterestForm().from_json(request.json)
            if not form.validate():
                field = []
                field_msg = []
                for err_key,err_val in form.errors.items():
            ...
            result = Interest.delete(self,**body)
    ...
    def get(self) -> Response:
        try:
            ...
            result = Interest.get_data(self,**body)
...
class InterestPaginateApi(Resource):
    def get(self) -> Response:
        try:
            default_config = current_app.config
            headers = request.headers
            ...
            result = Interest.get_paginate(self,**body)
