...
class CreateSettingsForm(Form):
    SettingsId = StringField('SettingsId', [validators.required()])
    Value = StringField('Value', [validators.required()])
    
class DeleteSettingsForm(Form):
    SettingsId = StringField('SettingsId', [validators.required()])

class SettingsPaginateApi(Resource):
    def get(self) -> Response:
        try:
            ...
            if not type(body) is dict:
                body = {}
            result = Settings.get_paginate(self,**body)
...
class SettingsApi(Resource):
    def post(self) -> Response:
        try:
            ...
            form = CreateSettingsForm().from_json(request.json)
            ...
            if not type(body) is dict:
                body = {}
            body['_id'] = body['SettingsId']
            del body['SettingsId']
            result = Settings.create(self,**body)
    ...
    def delete(self) -> Response:
        try:
            ...
            form = DeleteSettingsForm().from_json(request.json)
            if not form.validate():
                field = []
                field_msg = []
                for err_key,err_val in form.errors.items():
            ...
            result = Settings.delete(self,**body)
