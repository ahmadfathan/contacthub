...
class CreateBusinessTypeForm(Form):
    BusinessTypeId = StringField('BusinessTypeId', [validators.required()])
    
class DeleteBusinessTypeForm(Form):
    BusinessTypeId = StringField('BusinessTypeId', [validators.required()])
...
class BusinessTypeApi(Resource):
    def post(self) -> Response:
        try:
            ...
            form = CreateBusinessTypeForm().from_json(request.json)
            ...
            if not type(body) is dict:
                body = {}
            body['_id'] = body['BusinessTypeId']
            del body['BusinessTypeId']
            result = BusinessType.create(self,**body)
        ...
        except BusinessType.DoesNotExist as e:
            traceback.print_tb(e.__traceback__)
            telegram.send_log(request=request,message=str(e))
            return response.not_found_200(message=str(e))
    ...
    def delete(self) -> Response:
        try:
            ...
            form = DeleteBusinessTypeForm().from_json(request.json)
            if not form.validate():
                field = []
                field_msg = []
                for err_key,err_val in form.errors.items():
            ...
            result = BusinessType.delete(self,**body)
    ...
    def get(self) -> Response:
        try:
            ...
            result = BusinessType.get_data(self,**body)
...
class BusinessTypePaginateApi(Resource):
    def get(self) -> Response:
        try:
            default_config = current_app.config
            headers = request.headers
            ...
            result = BusinessType.get_paginate(self,**body)
