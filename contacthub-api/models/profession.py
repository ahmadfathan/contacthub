...
class Profession(Document):
    ...
    def create(self,_id:str,Description=None):
        if Profession.objects(_id=_id).first() is None:
            post_data = Profession(**{'_id' : _id,'Description':Description}).save()
        else:
            post_data = Profession.objects(_id=_id).update(set__Description=Description)

        if post_data:
            return Profession.get_data(self,**{'_id' : _id})[0]
        else:
            return False

    def delete(self,_id:str):
        return Profession.objects(_id=_id).delete()
    
    def get_data(self,**args_filter):
        pipeline = []
        pipeline.append({"$addFields" : {"ProfessionId" : "$_id"}})
        ...
        result = Profession.objects.aggregate(pipeline)
    ...
    def get_paginate(self,**args_filter):
        ...
        pipeline.append({"$addFields" : {"ProfessionId" : {"$toString" : "$_id"}}})
        ...
        result = Profession.objects.aggregate(*pipeline,allowDiskUse=True)
        result = json.loads(json_util.dumps(result))
