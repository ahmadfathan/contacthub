...
class Greeting(Document):
    ...
    def create(self,_id:str,Description=None):
        if Greeting.objects(_id=_id).first() is None:
            post_data = Greeting(**{'_id' : _id,'Description':Description}).save()
        else:
            post_data = Greeting.objects(_id=_id).update(set__Description=Description)

        if post_data:
            return Greeting.get_data(self,**{'_id' : _id})[0]
        else:
            return False

    def delete(self,_id:str):
        return Greeting.objects(_id=_id).delete()
    
    def get_data(self,**args_filter):
        pipeline = []
        pipeline.append({"$addFields" : {"GreetingId" : "$_id"}})
        for fil in args_filter:
            pipeline.append({"$match" : {fil : args_filter[fil]}})
        pipeline.append({"$project" : {
            "_id" : 0
        }})
        result = Greeting.objects.aggregate(pipeline)
    ...
    def get_paginate(self,**args_filter):
        ...
        pipeline.append({"$addFields" : {"GreetingId" : {"$toString" : "$_id"}}})
        ...
        result = Greeting.objects.aggregate(*pipeline,allowDiskUse=True)
        result = json.loads(json_util.dumps(result))
