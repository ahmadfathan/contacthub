...
class BusinessType(Document):
    ...
    def create(self,_id:str,Description=None):
        if BusinessType.objects(_id=_id).first() is None:
            post_data = BusinessType(**{'_id' : _id,'Description':Description}).save()
        else:
            post_data = BusinessType.objects(_id=_id).update(set__Description=Description)

        if post_data:
            return BusinessType.get_data(self,**{'_id' : _id})[0]
        else:
            return False

    def delete(self,_id:str):
        return BusinessType.objects(_id=_id).delete()
    
    def get_data(self,**args_filter):
        pipeline = []
        pipeline.append({"$addFields" : {"BusinessTypeId" : "$_id"}})
        ...
        result = BusinessType.objects.aggregate(pipeline)
    ...
    def get_paginate(self,**args_filter):
        ...
        pipeline.append({"$addFields" : {"BusinessTypeId" : {"$toString" : "$_id"}}})
        ...
        result = BusinessType.objects.aggregate(*pipeline,allowDiskUse=True)
        result = json.loads(json_util.dumps(result))
